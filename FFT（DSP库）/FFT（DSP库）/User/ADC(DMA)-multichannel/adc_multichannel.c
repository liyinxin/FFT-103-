...
__IO uint16_t ADC_ConvertedValue[2];         /*定义数组变量，分别放AD1通道10和11转换的数据*/
...
/**
  ...
  * @retval 无
  */
static void ADC1_MULTICHANNEL_GPIO_Config(void)
{
	...
	/* Configure PC.01、PC.00  as analog input */
...
/**
  ...
  * @retval 无
  */
static void ADC1_MULTICHANNEL_Mode_Config(void)
{
	...
	DMA_InitStructure.DMA_BufferSize = 2;                          //指定DMA通道的DMA缓存的大小，即需要开辟几个
	                                                               //内存空间，本次有两个转换通道
	DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;	//外设地址固定
	DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;  				//内存地址递加，即每次DMA都是将外设寄存器中的值传到两具内在空间中
	...
	ADC_InitStructure.ADC_ScanConvMode = ENABLE ; 	 				//     扫描 模式用于多通道采集
...
{
	ADC1_MULTICHANNEL_GPIO_Config();
	ADC1_MULTICHANNEL_Mode_Config();
}
